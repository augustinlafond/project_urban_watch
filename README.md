...
un modèle Random Forest entraîné sur 4,5 millions de pixels,
une API FastAPI permettant de demander une prédiction d’artificialisation à n’importe quelle coordonnée GPS.


📌 Objectif
L’objectif du projet est de produire un modèle capable de prédire la probabilité d’artificialisation (bâti) à partir d’une tuile Sentinel-2 300×300 pixels (≈ 3 km × 3 km) centrée autour d’un point GPS.
...
d’entraîner un modèle Random Forest sur > 4 500 000 pixels,
de restituer une carte complète 300×300 des prédictions du modèle,
...
Pour chaque coordonnée GPS, une bbox 3 km × 3 km est générée, puis :
SentinelHubRequest (SENTINEL2_L2A)
Résolution : 10 m
Fenêtre temporelle : ± 15 jours autour de la date demandée
...
Un masque nuageux est généré et les pixels nuageux sont remplacés par np.nan.
...
(d’où l’apparition naturelle de quelques 0 = NoData).
Les valeurs WorldCover sont ensuite converties :
50 = Built-up → 1
tout le reste → 0
6. 🤖 Modélisation : Random Forest
plus de 4 500 000 pixels utilisés pour l’entraînement
...
produire une prédiction 300×300 complète,
...
📊 Exemple de résultat
Entrée : image Sentinel-2 300×300
Sortie : carte binaire 300×300
Valeur = 1 → pixel urbain
Valeur = 0 → non urbain
...
F1-score : ~0.91
Accuracy : ~0.94
Ces résultats restent dépendants :
du lieu,
du taux de couverture nuageuse,
du choix de mosaicking SentinelHub,
du déséquilibre inhérent entre classes.
